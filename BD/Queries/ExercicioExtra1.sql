...
SELECT c.marca, c.modelo, SUM(v.distanciaPercorrida) FROM Carro c INNER JOIN empresa e ON c.idEmpresa = e.id 
INNER JOIN Viagem v ON v.idCarro = c.id GROUP BY c.marca, c.modelo, v.distanciaPercorrida ORDER BY v.distanciaPercorrida ASC;
 
--2)Apresentar nome das empresas cuja soma total da distância percorrida pelos carros,
--em viagens, é superior a 50000 km
SELECT e.nome FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id 
GROUP BY e.nome HAVING SUM(v.distanciaPercorrida) > 50000;
...
SELECT e.nome, SUM(v.distanciaPercorrida) AS somaDist, CONVERT(DECIMAL(7,2), AVG(v.distanciaPercorrida)) AS mediaDist 
FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id GROUP BY e.nome ORDER BY
mediaDist DESC;
 
--4)Apresentar nome das empresas cujos carro percorreram a maior distância dentre as cadastradas
SELECT e.nome FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id WHERE v.distanciaPercorrida IN
(SELECT MAX(distanciaPercorrida) FROM viagem);
...
SELECT e.nome, COUNT(c.id) AS quantCarros FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa
GROUP BY e.nome HAVING COUNT(c.id) >= 3 ORDER BY quantCarros DESC;
 
--6)Consultar Nomes das empresas que não tem carros cadastrados
SELECT e.nome FROM empresa e LEFT OUTER JOIN carro c ON e.id = c.idEmpresa WHERE c.idEmpresa IS NULL;
 
--7)Consultar Marca e modelos dos carros que não fizeram viagens
SELECT c.marca, c.modelo FROM viagem v RIGHT OUTER JOIN carro c ON v.idCarro = c.id WHERE v.idCarro IS NULL;
...
SELECT c.marca, c.modelo, COUNT(v.idCarro) FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id
GROUP BY c.marca, c.modelo, e.nome ORDER BY e.nome ASC, COUNT(v.idCarro); 
...
SELECT e.nome, c.marca, c.modelo, v.distanciaPercorrida, 
	CASE WHEN distanciaPercorrida > 1000
		THEN
			distanciaPercorrida * 15.00
		ELSE
			distanciaPercorrida * 10.00
		END AS valorGanho
FROM empresa e INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id;
...
SELECT e.nome, CONVERT(DECIMAL(7, 2), AVG(distanciaPercorrida)) AS mediaDistancia FROM empresa e 
INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id GROUP BY e.nome ORDER BY mediaDistancia ASC;
...
SELECT c.marca, c.modelo, e.nome, CONVERT(VARCHAR(10), v.data, 103) FROM empresa e 
INNER JOIN carro c ON e.id = c.idEmpresa INNER JOIN viagem v ON v.idCarro = c.id WHERE data IN 
(SELECT MAX(data) FROM viagem) 
...
SELECT c.marca, c.modelo, e.nome AS nome_empresa, COUNT(DISTINCT v.data) AS quantidade_dias FROM viagem v INNER JOIN carro c ON v.idCarro = c.id
INNER JOIN empresa e ON c.idEmpresa = e.id WHERE data BETWEEN '01/10/2022' AND '31/12/2022' GROUP BY c.marca, c.modelo, e.nome
ORDER BY quantidade_dias ASC, c.marca ASC, c.modelo ASC, e.nome ASC;