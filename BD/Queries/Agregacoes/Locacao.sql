...
SELECT c.num_cadastro, c.nome, f.titulo, FORMAT(d.data_fabricacao, 'dd/MM/yyyy') AS data_fabricacao, l.valor FROM Cliente c
JOIN Locacao l ON c.num_cadastro = l.Clientenum_cadastro JOIN DVD d ON l.DVDnum = d.num JOIN Filme f ON d.Filmeid = f.id
...
SELECT l.Clientenum_cadastro, c.nome, FORMAT(l.data_locacao, 'dd/MM/yyyy') AS data_locacao, COUNT(l.DVDnum) AS qtd FROM Locacao l
JOIN Cliente c ON l.Clientenum_cadastro = c.num_cadastro GROUP BY l.Clientenum_cadastro, c.nome, l.data_locacao;
...
SELECT l.Clientenum_cadastro, c.nome, FORMAT(l.data_locacao, 'dd/MM/yyyy') AS data_locacao, SUM(l.valor) AS valor_total FROM Locacao l
JOIN Cliente c ON l.Clientenum_cadastro = c.num_cadastro GROUP BY l.Clientenum_cadastro, c.nome, l.data_locacao;

-- 4) Consultar Consultar, num_cadastro do cliente, nome do cliente, Endereço
-- concatenado de logradouro e numero como Endereco, data de locação (Formato DD/MM/AAAA) dos clientes que alugaram mais de 2 filmes simultaneamente
SELECT l.Clientenum_cadastro, c.nome, CONCAT(c.logradouro, ', ', c.num) AS Endereco, FORMAT(l.data_locacao, 'dd/MM/yyyy') AS data_locacao FROM Locacao l
JOIN Cliente c ON l.Clientenum_cadastro = c.num_cadastro GROUP BY l.Clientenum_cadastro, c.nome, c.logradouro, c.num, l.data_locacao
