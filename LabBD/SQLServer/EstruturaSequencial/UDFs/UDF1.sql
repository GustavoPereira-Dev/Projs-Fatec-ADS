...
BEGIN
    INSERT INTO @tabela (Nome_Cliente, Nome_Produto, Quantidade, Valor_Total, Data_Hoje)
        SELECT c.Nome AS Nome_Cliente, p.Nome AS Nome_Produto, cp.Quantidade, 
...
SELECT * FROM fn_dadoscompra();-- 1. Criar uma database, criar as tabelas abaixo, definindo o tipo de dados e a relação PK/FK e popular com alguma massa de dados de teste (Suficiente para testar UDFs)
-- Funcionário (Código, Nome, Salário)
-- Dependendente (Código_Dep, Código_Funcionário, Nome_Dependente, Salário_Dependente)
CREATE DATABASE EmpresaDB;
GO
